...
calcPairAccel gConstant epsConstant target source
  ...
  | otherwise =
        let rVector = pos source ^-^ pos target --subtract source and target vector so there is a vector pointing to the source indicating gravitational pull
            ...
            ratio = (gConstant * mass source)/denominator
        in ratio *^ rVector

calcBodyAccel::R->R->[Body]->Body->Vector3 --calculate the acceleration on a body
calcBodyAccel gConstant epsConstant allBodies target = sumV $ map (calcPairAccel gConstant epsConstant target) allBodies --use sumV and map to get all of the pair accelerations and add them up

calcAllAccel::R->R->[Body]->[Vector3] --get all of the acceleration vectors
calcAllAccel gConstant epsConstant allBodies = map (calcBodyAccel gConstant epsConstant allBodies) allBodies
